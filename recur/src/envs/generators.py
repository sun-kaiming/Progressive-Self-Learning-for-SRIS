...
class Node():
    ...
    def val(self, series, deterministic=False):
        ...
        if self.value == 'mod':
            if self.children[1].val(series) == 0:
                return np.nan
            else:
                return self.children[0].val(series) % self.children[1].val(series)
        if self.value == 'div':
        ...
        if self.value.startswith('eval'):
            ...
            try:
                ...
                return getattr(scipy.special, self.value)(self.children[0].val(series))
...
class NodeList():
    ...
    def val(self, series, deterministic=False, dim_to_compute=None):
        ...
        return [self.nodes[i].val(series, deterministic=deterministic) if i in dim_to_compute else None for i in
    ...
    def get_formula_latex(self):
        return [node.qtree_prefix() for node in self.nodes]
...
class RandomRecurrence(Generator):
    def __init__(self, params):
        ...
        self.unaries_probabilities = np.array(
            [1.0 if o in self.extra_unary_operators or self.operators[o] > 0 else 0.0 for o in self.unaries])
        ...
        self.binaries_probabilities = np.array(
            [1.0 if o in self.extra_binary_operators or self.operators[o] > 0 else 0.0 for o in self.binaries])
        ...
        self.variables = ['n', 'rand'] + [f'x_{i}_{j}' for i in range(self.dimension) for j in
    ...
    def generate(self, rng, nb_ops=None, length=None, prediction_points=False, deg=None):
        ...
        if nb_ops is None:
            ...
            nb_ops = [rng.choice(a=[i + 1 for i in range(self.max_ops)], p=nb_ops_probas[dim]) for dim in
        ...
        initial_conditions = [[rng.uniform(-self.init_scale, self.init_scale) if self.float_sequences else rng.randint(
            -self.init_scale, self.init_scale + 1) \
        ...
        with open(f"./test_result/{name}.csv", 'a', encoding='utf-8-sig', newline='') as f:
            ...
            for i in range(n_predictions):
                try:
                    ...
                    src_tgt.extend(true)
                    errors.append(max([abs(float(p - t) / float(t + 1e-10)) for p, t in zip(pred, true)]))

                except:
                    writer.writerow(["," + ', '.join(str(id) for id in src), str(tgt), str(hyp), "", "", "预测错误"])
            ...
            for err in errors:
                if err != 0:
                    res = "预测错误"
                # str(tgt), str(hyp),
            writer.writerow(["," + ' '.join(str(id) for id in src), str(tgt), str(hyp),
                             "," + ' '.join(str(id) for id in src_tgt[-n_predictions:]),
                             "," + ' '.join(str(id) for id in src_hyp[-n_predictions:]), res])
    ...
    def evaluate_without_target(self, src, src_sta, hyp, n_predictions=3):
        errors = []
        n = n_predictions
        # print(src)
        for_len = len(src) - len(src_sta)
        if len(src) < n_predictions:
            n = int(len(src) / 3)
        # targets = src[-n:]
        # src = src[:-n]
        targets = src[len(src_sta):]
        # src = src[:6]
        ...
        with open(f"./test_result/{name}.csv", 'a', encoding='utf-8-sig', newline='') as f:
            ...
            for i in range(for_len):
                try:
                    ...
                    writer.writerow(["," + ', '.join(str(id) for id in src), "", str(hyp), "", "", "预测错误"])
                    return [-1 for _ in range(n_predictions)]
            ...
            writer.writerow(["," + ', '.join(str(id) for id in src[:-n]), "", str(hyp),
                             "," + ' '.join(str(id) for id in real_next_n),
                             "," + ' '.join(str(id) for id in pre_next_n), res])

            # print('input: ' + ', '.join( str(id) for id in src[:-n]))
            # print('预测公式: ' + str(hyp))
