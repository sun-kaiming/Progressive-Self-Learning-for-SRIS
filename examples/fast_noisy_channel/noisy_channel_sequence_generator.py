...
class NoisyChannelSequenceGenerator(object):
    ...
    def generate(
        ...
        def noisy_channel_rescoring(lprobs, beam_size, bsz, src_tokens, tokens, k):
            ...
            with torch.no_grad():
                ...
                if prefix_tokens is not None and step < prefix_tokens.size(1):
                    ...
                    cand_indices = prefix_tokens[:, step].view(-1, 1).expand(bsz, beam_size).data.contiguous().view(
                        bsz * beam_size, 1)
                ...
                src_size = torch.sum(src_tokens[:, :] != self.src_dict.pad_index, dim=1, keepdim=True,
                ...
                if self.combine_method != "lm_only":
                    ...
                    if step == 0:
                        ...
                        channel_input = torch.cat(
                            (tokens[:, 1:step + 1].repeat(1, k).view(-1, step), fw_top_k_idx.view(-1, 1), eos_tokens),
                            1)
                    ...
                    if self.channel_scoring_type == "unnormalized":
                        ...
                        ch_decoder_output, _ = channel_model.decoder(temp_src_tokens_full,
                                                                     encoder_out=ch_encoder_output, features_only=True)
                        del ch_encoder_output
                        ch_intermed_scores = channel_model.decoder.unnormalized_scores_given_target(ch_decoder_output,
                                                                                                    target_ids=temp_src_tokens_full[
                                                                                                               :, 1:])
                        ...
                        for k_idx in range(k):
                            ...
                            if step == 0:
                                ...
                                k_ch_input = torch.cat(
                                    (tokens[:, 1:step + 1], fw_top_k_idx[:, k_idx:k_idx + 1], k_eos_tokens), 1)
                            ...
                            k_ch_intermed_scores = torch.gather(k_ch_lprobs[:, :-1, :], 2,
                        ...
                        for i, start_idx in enumerate(range(0, ch_bsz_size, self.ch_scoring_bsz)):
                            ...
                            ch_encoder_output_batch = channel_model.encoder(channel_input_batch,
                                                                            src_lengths=ch_input_lengths_batch)
                            ch_decoder_output_batch, _ = channel_model.decoder(temp_src_tokens_full_batch,
                        ...
                        ch_intermed_scores = torch.gather(ch_lprobs[:, :-1, :], 2,
                                                          temp_src_tokens_full[:, 1:].unsqueeze(2)).squeeze().view(
                            bsz * beam_size * k, -1)
                        ch_intermed_scores *= not_padding.float()
                        ch_scores = torch.sum(ch_intermed_scores, dim=1)
                ...
                if self.share_tgt_dict:
                    lm_scores = get_lm_scores(lm, tokens[:, :step + 1].view(-1, step + 1), lm_incremental_states,
                                              fw_top_k_idx.view(-1, 1), torch.tensor(np.full(tokens.size(0), step + 1)),
                                              k)
                else:
                    ...
                    lm_scores = get_lm_scores(lm, new_lm_input, lm_incremental_states, new_cands,
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            fw_lprobs, ch_lm_lprobs, lm_lprobs = noisy_channel_rescoring(fw_lprobs, beam_size, bsz, src_tokens, tokens,
                                                                         self.k2)
...
def get_lm_scores(model, input_tokens, incremental_states, cand_tokens, input_len, k):
    with torch.no_grad():
        ...
        probs_next_wrd = torch.gather(lm_lprobs.repeat(1, k).view(lm_lprobs_size * k, -1), 1,
                                      cand_tokens).squeeze().view(-1)
