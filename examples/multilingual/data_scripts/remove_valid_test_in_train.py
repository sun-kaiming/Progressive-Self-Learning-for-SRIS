...
def train_size_if_remove_in_otherset(data_sizes, mess_up_train):
    ...
    for direction, count in counts_in_other.items():
        remain_sizes.append((direction, data_sizes[direction] - count, data_sizes[direction], count,
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(data_situation,
                      columns=['direction', 'train_size_after_remove', 'orig_size', 'num_to_remove', 'remove_percent'])
    ...
    real_data_situation = []
    for direction in directions:
        org_size, new_size = remove_messed_up_sentences(raw_data, direction, mess_up_train, all_messed_pairs,
    ...
    df = pd.DataFrame(real_data_situation,
                      columns=['direction', 'train_size_after_remove', 'orig_size', 'num_to_remove', 'remove_percent'])
    df.sort_values('remove_percent', ascending=False)
