...
class GuidedCrossEntAccCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        is_dual_input = True if net_input['src_tokens'] is not None and net_input.get(
            'src_txt_tokens') is not None else False
        ...
        if is_dual_input:
            ...
            text_loss, text_nll_loss, text_correct, text_total = self.compute_loss_and_acc(model, lprobs_text, target,
                                                                                           reduction=reduction)
        ...
        sample_size, logging_output = self.get_logging_output(
            sample, loss, nll_loss, correct, total, src_token_num, speech_loss, speech_nll_loss, attn_cost,
            is_dual_input
        )
    ...
    def aggregate_logging_outputs(logging_outputs):
        ...
        agg_output = {
            ...
            "speech_nll_loss": speech_nll_loss_sum / sample_size_speech / math.log(
                2) if sample_size_speech > 0 else 0.0,
