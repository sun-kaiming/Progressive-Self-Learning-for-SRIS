...
class HeadSelectionS2TTransformerModel(S2TTransformerModel):
    ...
    def build_decoder(cls, args, task, embed_tokens):
        if (safe_hasattr(args, "decoder_self_attn_head_select") and args.decoder_self_attn_head_select) or (
                safe_hasattr(args, "dec_enc_attn_head_select") and args.dec_enc_attn_head_select):
...
class HeadSelectionS2TTransformerEncoder(S2TTransformerEncoder):
    ...
    def __init__(self, args):
        ...
        self.transformer_layers = nn.ModuleList([
            HeadSelectionTransformerEncoderLayer(args, layer_idx, attn_head_selector=self.attn_head_selector) for
            layer_idx in range(args.encoder_layers)
        ])
