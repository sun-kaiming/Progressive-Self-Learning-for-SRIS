...
class MultiheadAttentionSelection(MultiheadAttention):
    ...
    def forward(
        ...
        if subset_heads is None:
            ...
            attn = torch.stack(
                [mixed_attn[torch.arange(bsz), subset_heads[:, col], :, :] for col in range(subset_heads.size(1))],
                dim=1
            )
        ...
        if need_weights:
            if subset_heads is None:
                ...
                attn_weights = torch.stack(
                    [mixed_attn_weights[torch.arange(bsz), subset_heads[:, col], :, :] for col in
                     range(subset_heads.size(1))], dim=1
                ).transpose(1, 0)
