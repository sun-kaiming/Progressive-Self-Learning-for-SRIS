...
class Evaluator(object):
    ...
    def enc_dec_step_beam(self, data_type, task, scores):
        ...
        for (x1, len1), (x2, len2), trees, infos in iterator:
            ...
            valid_per_info_type_per_minibatch = {info_type: torch.zeros(n_infos_prior, bs, dtype=torch.long) for
                                                 info_type in info_types}
        ...
        for (x1, len1), (x2, len2), trees, infos in iterator:
            ...
            for i in range(len(len1)):
                ...
                if method == "lagrange":
                    ...
                    interp = np.polynomial.chebyshev.Chebyshev.fit(range(input_length), src, order,
                                                                   domain=[0, input_length + output_length - 1])
                elif method == "polyfit":
